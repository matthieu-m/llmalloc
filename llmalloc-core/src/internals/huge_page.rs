...
use core::{cmp, mem, num, ptr, slice, sync::atomic};
...
impl HugePage {
    ...
    pub(crate) unsafe fn allocate(&self, bytes: usize) -> *mut u8 {
        let large_page_size = self.common.page_size;

        let number_pages = large_page_size.round_up(bytes) / large_page_size;
        debug_assert!(number_pages > 0);
        debug_assert!(number_pages <= self.common.number_pages.0);

        if let Some(index) = self.foreign.allocate(NumberPages(number_pages)) {
...
impl Foreign {
    ...
    unsafe fn allocate(&self, number_pages: NumberPages) -> Option<PageIndex> {
        if number_pages.0 == 1 {
            self.fast_allocate()
        } else {
            self.flexible_allocate(number_pages)
        }
    }
    ...
    //  Internal: allocate multiple pages, if possible.
    fn flexible_allocate(&self, number_pages: NumberPages) -> Option<PageIndex> {
        let index = self.pages.flexible_allocate(number_pages);
...
impl PageTokens {
    ...
    fn flexible_allocate(&self, number_pages: NumberPages) -> Option<PageIndex> {
        ...
        while outer >= 0 {
            ...
            let index = unsafe { self.flexible_allocate_backward_from(outer as usize, number_pages) };
    ...
    //
    //  -   `outer` is within bounds.
    unsafe fn flexible_allocate_backward_from(&self, outer: usize, number_pages: NumberPages)
        -> Result<PageIndex, isize>
    {
        ...
        let (inner, tail) =
            match self.0.get_unchecked(outer).claim_multiple(cmp::min(number_pages, BitMask::CAPACITY)) {
                Some(tuple) => tuple,
...
impl BitMask {
    ...
    fn claim_multiple(&self, number: usize) -> Option<(usize, usize)> {
        //  Do a single pass over the bits, attempting to find `number` consecutive ones.
        ...
        loop {
            ...
            let candidate = potential - number;
        ...
        //  Claim as many low-bits as possible.
        {
            let current_mask = self.0.load(atomic::Ordering::Acquire);

            let potential = current_mask.trailing_zeros() as usize;
...
fn claim_multiple(mask: u64, number: usize) -> (Option<usize>, Option<usize>, u64) {
    ...
    if let Some(claimed) = bitmask.claim_multiple(number) {
...
fn flexible_allocate(number: usize, initial: RawPageTokens, expected: RawPageTokens) -> Option<usize> {
    let tokens = create_page_tokens(initial);
    let index = tokens.flexible_allocate(NumberPages(number)).map(|x| x.0.get());
...
fn foreign_allocate_deallocate_fast() {
    fn allocate_fast(foreign: &Foreign) -> Option<usize> {
        unsafe { foreign.allocate(NumberPages(1)) }.map(|x| x.0.get())
    }
...
fn foreign_allocate_deallocate_flexible() {
    fn allocate_flexible(foreign: &Foreign, number_pages: usize) -> Option<usize> {
        unsafe { foreign.allocate(NumberPages(number_pages)) }.map(|x| x.0.get())
    }
...
fn huge_page_smoke_test() {
    ...
    let allocated = unsafe { huge_page.allocate(LARGE_PAGE_SIZE + 1) };
    ...
    let failed = unsafe { huge_page.allocate(LARGE_PAGE_SIZE * 14) };
